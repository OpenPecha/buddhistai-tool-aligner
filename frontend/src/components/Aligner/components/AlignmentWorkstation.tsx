...
function AlignmentWorkstationContent() {
  ...
  React.useEffect(() => {
    const loadFromUrl = async (sid: string, tid: string) => {
      try {
        ...
        if (preparedData.has_alignment && preparedData.annotation?.data) {
          const annotationData = (
            preparedData.annotation.data as unknown as {
              target_annotation: Array<{ index: string; span: { start: number; end: number } }>;
              alignment_annotation: Array<{ index: string; span: { start: number; end: number }; alignment_index: string[] }>;
            }
          );
  ...
  return (
    <div className='w-full h-full flex flex-col min-h-0'>
      ...
      {/* Editor View - Show when both texts are loaded */}
      {bothTextLoaded ? (
