...
function UnifiedSelectionPanel() {
  ...
  const handleBdrcResultSelect = React.useCallback(async (result: { workId?: string }) => {
  ...
  const handleTargetInstanceSelection = React.useCallback(async (instanceId: string, hasAlignment: boolean) => {
    if (!instanceId) {
    ...
    if (!hasAlignment) {
      ...
      try {
        ...
        const getSegmentationAnnotationId = (instanceData: OpenPechaTextInstance): string | null => {
          if (!instanceData?.annotations || typeof instanceData.annotations !== 'object') {
            return null;
          }
          ...
          const annotation = Object.values(instanceData.annotations)
            .flat()
            .find((ann): ann is AnnotationWithId => 
              ann !== null && 
              typeof ann === 'object' && 
        ...
        const sourceSegmentationAnnotationId = getSegmentationAnnotationId(sourceInstanceData);
        const targetSegmentationAnnotationId = getSegmentationAnnotationId(targetInstanceData);

        // Fetch segmentation annotations if available
        let sourceSegmentation: Array<{span: {start: number, end: number}}> | null = null;
        let targetSegmentation: Array<{span: {start: number, end: number}}> | null = null;
        ...
        const extractSegmentationArray = (annotationData: unknown): Array<{span: {start: number, end: number}}> | null => {
        ...
        if (sourceSegmentationAnnotationId) {
          try {
            const sourceAnnotationData = await fetchAnnotation(sourceSegmentationAnnotationId);
            sourceSegmentation = extractSegmentationArray(sourceAnnotationData);
          } catch (error) {
        ...
        if (targetSegmentationAnnotationId) {
          try {
            const targetAnnotationData = await fetchAnnotation(targetSegmentationAnnotationId);
            targetSegmentation = extractSegmentationArray(targetAnnotationData);
          } catch (error) {
            console.warn('Failed to fetch target segmentation annotation:', error);
          }
        }

        // If no segmentation annotation found, try extracting from instance annotations
        sourceSegmentation ??= extractInstanceSegmentation(sourceInstanceData.annotations);
        targetSegmentation ??= extractInstanceSegmentation(targetInstanceData.annotations);
        ...
        if (sourceSegmentation && sourceSegmentation.length > 0) {
          segmentedSourceText = applySegmentation(sourceContent, sourceSegmentation);
        } else {
        ...
        if (targetSegmentation && targetSegmentation.length > 0) {
          segmentedTargetText = applySegmentation(targetContent, targetSegmentation);
        } else {
          // Fallback to file segmentation if no instance segmentation
  ...
  }, [relatedInstances, handleChangeSearchParams, sourceInstanceId, sourceTextId, setSourceText, setTargetText, setLoadingAnnotations, setAnnotationsApplied]);
  ...
  const shouldShowInstanceSelector = hasSelectedText && selectedTextId && !isCheckingBdrcText;
  const shouldShowRelatedInstances = hasSelectedText && selectedTextId && !isCheckingBdrcText && sourceInstanceId;
          ...
          {shouldShowRelatedInstances && (
            <>
              <RelatedInstancesPanel
                ...
                onCreateTranslation={() => handleActionSelection('create-translation')}
                onCreateCommentary={() => handleActionSelection('create-commentary')}
              />
